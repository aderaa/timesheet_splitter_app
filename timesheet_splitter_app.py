...
TEXT = {
    "en": {
        ...
        "subtitle": "Upload the full Clarity timesheet export and the vendor database. The app will create one Excel file per employee, grouped under folders per vendor, plus a DOCX summary report.",
        "settings": "Settings",
        ...
        "doc_section_ignored": "4. Ignored employees (not in vendor database)",
        ...
        "doc_overview_bullet_5": "Ignored employees (not in DB): {ignored}",
    ...
    "ar": {
        ...
        "subtitle": "قم برفع ملف الجداول الزمنية الكامل من Clarity وملف قاعدة بيانات الموردين، وستقوم الأداة بإنشاء ملف Excel لكل موظف داخل مجلد باسم المورد مع تقرير ملخص بصيغة DOCX.",
        "settings": "الإعدادات",
        ...
        "metric_ignored_emps": "الموظفون المتجاهلون (غير موجودين في قاعدة البيانات)",
        ...
        "ignored_table_title": "الموظفون المتجاهلون",
        ...
        "doc_section_ignored": "٤. الموظفون المتجاهلون (غير موجودين في قاعدة بيانات الموردين)",
        ...
        "doc_overview_bullet_5": "عدد الموظفين المتجاهلين (غير موجودين في قاعدة البيانات): {ignored}",
...
def prepare_employee_data(full_df: pd.DataFrame, db_df: pd.DataFrame):
    """
    Build in-memory structures for employees:
    - employees: list of dicts with vendor, emp_id, emp_name, hours, df
    - ignored: employees present in full_df but not in db_df
    """
    ...
    if DB_EMP_ID_COL not in db_df.columns or DB_VENDOR_COL not in db_df.columns:
        raise ValueError("Vendor DB is missing required columns.")
    ...
    return employees, ignored, unique_emp_ids


def build_summary_structures(employees, ignored, failed, full_df, lang: str):
    ...
    return summary_stats, vendor_summary_df, exported_df, ignored_df, failed_df


def build_docx_summary(summary_stats, vendor_summary_df, exported_df, ignored_df, failed_df, lang: str) -> Document:
...
def main():
    ...
    try:
        ...
        employees, ignored, unique_emp_ids = prepare_employee_data(full_df, db_df)
        ...
        # Use successful_employees for summary, not those that failed
        employees_for_summary = []
        # Map back: we had full dicts in `employees`, but we only want hours + meta for successful ones
        ...
        summary_stats, vendor_summary_df, exported_df, ignored_df, failed_df = build_summary_structures(
            employees_for_summary, ignored, failed, full_df, lang
        )

        # Build DOCX summary
        doc = build_docx_summary(summary_stats, vendor_summary_df, exported_df, ignored_df, failed_df, lang)
        ...
        st.markdown("----")
        c5, c6 = st.columns(2)
        c5.write(f"**{t('run_timestamp', lang)}:** {summary_stats['run_timestamp'].strftime('%Y-%m-%d %H:%M:%S')}")
        c6.write(f"**{t('period', lang)}:** {summary_stats['period']}")
